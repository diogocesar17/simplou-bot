  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    if (!msg.message || !msg.message.conversation) return;
      ...
      if (textoLower.startsWith("resumo detalhado")) {
        ...
        const todos = await listarLancamentos(userId, 9999, parsed.mes, parsed.ano);
      ...
      // Comando para listar parcelados ativos
      ...
      // Comando para listar recorrentes ativos
      ...
      // Comando para próximos vencimentos
      if (["vencimentos", "vencimento"].includes(textoLower)) {
        try {
          const vencimentos = await buscarProximosVencimentos(userId, 30);
          
          let msgVencimentos = '📅 *Próximos Vencimentos (30 dias):*\n\n';
          ...
          if (!temVencimentos) {
            msgVencimentos = '📅 Nenhum vencimento nos próximos 30 dias.';
          } else {
      ...
      // Comando para vencimentos com período específico
      if (/^vencimentos\s+(\d+)$/i.test(textoLower)) {
        try {
          const match = textoLower.match(/^vencimentos\s+(\d+)$/i);
      ...
      // Comando para gastos por categoria específica
      if (/^categoria\s+(.+)$/i.test(textoLower)) {
        try {
          const match = textoLower.match(/^categoria\s+(.+)$/i);
      ...
      // Comando para gastos com valor alto
      if (/^valor alto\s*(\d*)$/i.test(textoLower)) {
        try {
          const match = textoLower.match(/^valor alto\s*(\d*)$/i);
          const valorMinimo = match[1] ? parseInt(match[1]) : 100;
