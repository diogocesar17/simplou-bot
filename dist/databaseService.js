...
exports.appendRowToDatabase = appendRowToDatabase;
...
exports.formatarValor = formatarValor;
...
exports.getTotalGastosPorPagamento = getTotalGastosPorPagamento;
...
exports.buscarBoletosVencimentoProximo = buscarBoletosVencimentoProximo;
exports.buscarAlertasDoDia = buscarAlertasDoDia;
exports.formatarAlertaCartao = formatarAlertaCartao;
exports.formatarAlertaBoleto = formatarAlertaBoleto;
exports.gerarMensagemAlertas = gerarMensagemAlertas;
...
pool.on('connect', (client) => {
    logger_1.logger.info('🔌 Nova conexão PostgreSQL estabelecida');
});
pool.on('error', (err, client) => {
    logger_1.fileLogger.error('❌ Erro no pool PostgreSQL:', err.message);
});
// Inicializar tabelas se não existirem
async function initializeDatabase() {
    try {
        logger_1.logger.info('🔌 Tentando conectar ao PostgreSQL...');
        if (!process.env.DATABASE_URL) {
            throw new Error('DATABASE_URL não configurada. Verifique as variáveis de ambiente.');
        }
        const client = await pool.connect();
        logger_1.logger.info('✅ Conectado ao PostgreSQL com sucesso');
    ...
    `);
        ...
        client.release();
        logger_1.logger.info('✅ Banco de dados inicializado com sucesso');
    }
    catch (error) {
        logger_1.fileLogger.error('❌ Erro ao inicializar banco de dados:', error.message);
        logger_1.logger.error('🔍 Verifique se:');
        logger_1.logger.error('   - DATABASE_URL está configurada');
        logger_1.logger.error('   - PostgreSQL está rodando');
        logger_1.logger.error('   - Credenciais estão corretas');
        throw error;
    }
}
...
async function migrateCartaoColumns(client) {
    ...
    `);
        ...
        if (columnsToAdd.length > 0) {
            logger_1.logger.info('🔄 Migrando colunas de cartão de crédito...');
            for (const column of columnsToAdd) {
                ...
                if (columnDefinition) {
                    ...
                    logger_1.logger.info(`✅ Coluna ${column} adicionada`);
                }
            }
            logger_1.logger.info('✅ Migração de colunas concluída');
        }
        else {
            logger_1.logger.info('✅ Colunas de cartão já existem');
        }
    }
    catch (error) {
        logger_1.fileLogger.error('❌ Erro na migração:', error.message);
        throw error;
    }
}
async function migrateParcelamentoRecorrenteColumns(client) {
    ...
    `);
        ...
        const columnsToAdd = newColumns.filter(col => !existingColumns.includes(col));
        if (columnsToAdd.length > 0) {
            logger_1.logger.info('🔄 Migrando colunas de parcelamento e recorrente...');
            for (const column of columnsToAdd) {
                let columnDefinition = '';
                switch (column) {
            ...
            logger_1.logger.info('✅ Migração de colunas de parcelamento e recorrente concluída');
        }
        else {
            logger_1.logger.info('✅ Colunas de parcelamento e recorrente já existem');
        }
    }
    catch (error) {
        logger_1.fileLogger.error('❌ Erro na migração de parcelamento e recorrente:', error.message);
        throw error;
    }
}
async function migrateCartoesConfigTable(client) {
    ...
    `);
        if (checkColumn.rows.length === 0) {
            logger_1.logger.info('🔄 Adicionando coluna dia_fechamento à tabela cartoes_config...');
            await client.query(`ALTER TABLE cartoes_config ADD COLUMN dia_fechamento INTEGER`);
            logger_1.logger.info('✅ Coluna dia_fechamento adicionada à tabela cartoes_config');
        }
        else {
            logger_1.logger.info('✅ Coluna dia_fechamento já existe na tabela cartoes_config');
        }
    }
    catch (error) {
        logger_1.fileLogger.error('❌ Erro na migração da tabela cartoes_config:', error.message);
        throw error;
    }
}
async function migrateDataVencimentoColumn(client) {
    ...
    `);
        if (checkColumn.rows.length === 0) {
            logger_1.logger.info('🔄 Adicionando coluna data_vencimento à tabela lancamentos...');
            await client.query(`ALTER TABLE lancamentos ADD COLUMN data_vencimento DATE`);
            logger_1.logger.info('✅ Coluna data_vencimento adicionada à tabela lancamentos');
        }
        else {
            logger_1.logger.info('✅ Coluna data_vencimento já existe na tabela lancamentos');
        }
    }
    catch (error) {
        logger_1.fileLogger.error('❌ Erro na migração da coluna data_vencimento:', error.message);
        throw error;
    }
}
async function migrateTimezoneColumns(client) {
    ...
    `);
        ...
        const columnsToAdd = newColumns.filter(col => !existingColumns.includes(col));
        if (columnsToAdd.length > 0) {
            logger_1.logger.info('🔄 Migrando colunas de timestamp para correção de timezone...');
            for (const column of columnsToAdd) {
                let columnDefinition = '';
                switch (column) {
                    case 'criado_em':
            ...
            logger_1.logger.info('✅ Migração de colunas de timestamp concluída');
        }
        else {
            logger_1.logger.info('✅ Colunas de timestamp já estão corretas');
        }
    }
    catch (error) {
        logger_1.fileLogger.error('❌ Erro na migração de colunas de timestamp:', error.message);
        throw error;
    }
}
...
async function salvarConfiguracaoCartao(userId, nomeCartao, diaVencimento, diaFechamento = null) {
    ...
    `;
        ...
        logger_1.fileLogger.error('❌ Erro ao salvar configuração de cartão:', error);
        throw error;
    }
}
async function atualizarCartaoConfigurado(userId, nomeCartao, diaVencimento, diaFechamento = null) {
    ...
    `;
        ...
        logger_1.fileLogger.error('❌ Erro ao atualizar configuração de cartão:', error);
        throw error;
    }
}
...
async function contarLancamentosAssociadosCartao(userId, nomeCartao) {
    ...
    `;
        ...
        logger_1.fileLogger.error('❌ Erro ao contar lançamentos associados:', error);
        throw error;
    }
}
async function excluirCartaoConfigurado(userId, nomeCartao) {
    ...
    `;
        ...
        logger_1.fileLogger.error('❌ Erro ao excluir cartão:', error);
        throw error;
    }
}
...
async function appendRowToDatabase(userId, values) {
    ...
    `;
        ...
        logger_1.fileLogger.error('❌ Erro ao adicionar lançamento:', error);
        throw error;
    }
}
...
async function getResumoPorMes(userId, mes, ano) {
    logger_1.logger.debug('[DEBUG] getResumoPorMes chamada com:', { userId, mes, ano });
    const query = `
    SELECT 
      tipo,
      SUM(valor) as total,
  ...
  `;
    ...
    logger_1.logger.debug('[DEBUG] Query params:', params);
    logger_1.logger.debug('[DEBUG] Query SQL:', query);
    const result = await pool.query(query, params);
    logger_1.logger.debug('[DEBUG] Resultado da query:', result.rows);
    let totalReceitas = 0;
    let totalDespesas = 0;
    let totalLancamentos = 0;
    result.rows.forEach(row => {
        logger_1.logger.debug('[DEBUG] Processando row:', row);
        if (row.tipo === 'receita') {
            totalReceitas = parseFloat(row.total);
        }
        else if (row.tipo === 'gasto') {
    ...
    const saldo = totalReceitas - totalDespesas;
    logger_1.logger.debug('[DEBUG] Totais calculados:', { totalReceitas, totalDespesas, saldo, totalLancamentos });
    return {
        totalReceitas,
        totalDespesas,
        saldo,
...
async function listarLancamentos(userId, limite = 20, mes = null, ano = null) {
  ...
  `;
    ...
    for (let l of lancamentos) {
        ...
        if (lancamentosAgrupados.length >= limite)
            break;
    }
...
async function atualizarLancamentoPorId(userId, id, novosDados) {
    try {
      ...
      RETURNING id
    `;
        logger_1.logger.debug('[DEBUG] Query de atualização:', query);
        logger_1.logger.debug('[DEBUG] Valores:', valores);
        ...
        logger_1.fileLogger.error('❌ Erro ao atualizar lançamento:', error);
        throw error;
    }
}
async function excluirLancamentoPorId(userId, id) {
    try {
        ...
        logger_1.fileLogger.error('❌ Erro ao excluir lançamento:', error);
        throw error;
    }
}
...
async function excluirParcelamentoPorId(userId, parcelamentoId) {
    ...
    `;
        ...
        logger_1.fileLogger.error('❌ Erro ao excluir parcelamento:', error);
        throw error;
    }
}
...
async function excluirRecorrentePorId(userId, recorrenteId) {
    ...
    `;
        ...
        logger_1.fileLogger.error('❌ Erro ao excluir recorrente:', error);
        throw error;
    }
}
...
async function queryDatabase(query, params = []) {
    try {
        ...
        logger_1.fileLogger.error('[DATABASE] Erro na query:', error);
        throw error;
    }
}
...
async function registrarLog(userId, acao, detalhes = null) {
    ...
    `;
        ...
        logger_1.fileLogger.error('[LOGS] Erro ao registrar log:', error);
    }
}
/**
...
async function buscarLogsRecentes(limite = 10) {
    ...
    `;
        ...
        logger_1.fileLogger.error('[LOGS] Erro ao buscar logs:', error);
        return [];
    }
}
/**
...
async function gerarEstatisticasSistema() {
    ...
    `;
        ...
        logger_1.fileLogger.error('[STATS] Erro ao gerar estatísticas:', error);
        throw error;
    }
}
/**
...
async function gerarBackupCSV(userId) {
    ...
    `;
        ...
        const nomeArquivo = `backup_${userId}_${timestamp}.csv`;
        ...
        logger_1.fileLogger.error('[BACKUP] Erro ao gerar backup:', error);
        return {
            sucesso: false,
            erro: error.message
        };
...
async function limparDadosAntigos() {
    ...
    `;
        ...
        logger_1.fileLogger.error('[LIMPEZA] Erro ao limpar dados:', error);
        throw error;
    }
}
/**
...
async function gerarLogAuditoria(limite = 100) {
    ...
    `;
        ...
        logger_1.fileLogger.error('[LOGS] Erro ao gerar log de auditoria:', error);
        throw error;
    }
}
...
async function buscarParceladosAtivos(userId, limite = 20) {
    ...
    `;
        ...
        logger_1.fileLogger.error('Erro ao buscar parcelados ativos:', error);
        throw error;
    }
}
...
async function buscarRecorrentesAtivos(userId, limite = 20) {
    ...
    `;
        ...
        logger_1.fileLogger.error('Erro ao buscar recorrentes ativos:', error);
        throw error;
    }
}
...
async function buscarProximosVencimentos(userId, dias = 30) {
    ...
    `;
        ...
        logger_1.fileLogger.error('Erro ao buscar próximos vencimentos:', error);
        throw error;
    }
}
...
async function buscarGastosPorCategoria(userId, categoria, limite = 20, mes = null, ano = null) {
    ...
    `;
        ...
        logger_1.fileLogger.error('Erro ao buscar gastos por categoria:', error);
        throw error;
    }
}
...
async function buscarGastosValorAlto(userId, valorMinimo = 100, limite = 20, mes = null, ano = null) {
    ...
    `;
        ...
        logger_1.fileLogger.error('Erro ao buscar gastos com valor alto:', error);
        throw error;
    }
}
// ===== FUNÇÕES DE USUÁRIOS =====
...
async function migrarUsuariosConfig(client) {
    try {
        ...
        if (parseInt(checkResult.rows[0].total) > 0) {
            logger_1.logger.info('✅ Usuários já migrados, pulando migração');
            return;
        }
        logger_1.logger.info('🔄 Migrando usuários do config.js...');
      ...
      `, [adminId, 'Administrador', 'premium', 'ativo', true, 'sistema']);
            logger_1.logger.info(`✅ Admin migrado: ${adminId}`);
        }
        ...
        `, [userId, 'Usuário', 'gratuito', 'ativo', false, 'sistema']);
                logger_1.logger.info(`✅ Usuário migrado: ${userId}`);
            }
        }
        logger_1.logger.info('✅ Migração de usuários concluída');
    }
    catch (error) {
        logger_1.logger.error('❌ Erro na migração de usuários:', error);
        throw error;
    }
}
...
async function cadastrarUsuario(userId, dados) {
    ...
    `, [userId, nome, plano, 'ativo', is_admin, criado_por]);
        ...
        logger_1.logger.info(`✅ Usuário cadastrado: ${userId} (${nome})`);
        return result.rows[0];
    }
    catch (error) {
        ...
        logger_1.logger.error('❌ Erro ao cadastrar usuário:', error);
        throw error;
    }
}
...
async function promoverParaPremium(userId, diasExpiracao = null, promovidoPor) {
    ...
    `, [userId, dataExpiracao]);
        ...
        logger_1.logger.info(`✅ Usuário promovido para premium: ${userId}`);
        return result.rows[0];
    }
    catch (error) {
        logger_1.logger.error('❌ Erro ao promover usuário:', error);
        throw error;
    }
}
...
async function removerUsuario(userId, removidoPor) {
    ...
    `, [userId]);
        ...
        logger_1.logger.info(`✅ Usuário removido: ${userId}`);
        return usuario;
    }
    catch (error) {
        logger_1.logger.error('❌ Erro ao remover usuário:', error);
        throw error;
    }
}
...
async function listarUsuarios(filtros = {}) {
    try {
        ...
        logger_1.logger.error('❌ Erro ao listar usuários:', error);
        throw error;
    }
}
...
async function verificarAcessoUsuario(userId) {
    try {
        ...
        logger_1.logger.error('❌ Erro ao verificar acesso:', error);
...
async function registrarAcesso(userId) {
    ...
    `, [userId]);
        ...
        logger_1.logger.error('❌ Erro ao registrar acesso:', error);
    }
}
...
async function buscarUsuariosPremiumExpiracao(diasAntes = 7) {
    ...
    `, [dataLimite]);
        ...
        logger_1.logger.error('❌ Erro ao buscar usuários premium próximos da expiração:', error);
        throw error;
    }
}
