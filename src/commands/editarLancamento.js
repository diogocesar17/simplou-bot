...
async function editarLancamentoCommand(sock, userId, texto) {
  ...
  if (aguardandoEdicao[userId]) {
    ...
    try {
      // Atualizar no banco
      await lancamentosService.atualizarLancamento(userId, contexto.lancamentoId, novosDados);
  ...
  // Armazenar contexto para edição
  aguardandoEdicao[userId] = {
  ...
  const processarEscolha = async (sock, userId, texto) => {
    if (aguardandoEscolha[userId]) {
      const contexto = aguardandoEscolha[userId];
      delete aguardandoEscolha[userId];
...
module.exports = editarLancamentoCommand; 