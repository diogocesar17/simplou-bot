...
const { parseMessage } = require('../utils/parseUtils');
...
async function criarParcelamento(userId, parsed, cartaoInfo = null) {
  ...
  for (let i = 0; i < parsed.numParcelas; i++) {
    ...
    const dados = {
      ...
      data_contabilizacao: cartaoInfo ? new Date().toISOString().split('T')[0] : null,
      mes_fatura: cartaoInfo ? new Date().getMonth() + 1 : null,
      ano_fatura: cartaoInfo ? new Date().getFullYear() : null,
...
function gerarMensagemSucesso(parsed) {
...
async function lancamentoCommand(sock, userId, texto) {
  ...
  if (estado?.etapa === 'aguardando_escolha_cartao') {
    ...
    parsed.cartaoEscolhido = cartoes[escolha];
    return await processarLancamento(sock, userId, parsed);
  }
  ...
  if (parsed.tipo && parsed.tipo.toLowerCase() === 'gasto' && 
    ...
    if (cartoes.length === 1) {
      ...
      const dados = {
        ...
        data_contabilizacao: new Date().toISOString().split('T')[0],
      ...
      await lancamentosService.salvarLancamento(userId, dados);
      await sock.sendMessage(userId, { text: gerarMensagemSucesso(parsed) });
      return;
    }
...
module.exports = lancamentoCommand;