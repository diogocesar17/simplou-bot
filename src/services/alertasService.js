 ...
 * @param {string} tipoAlerta - Tipo do alerta (vencimento/premium)
 * @param {string} identificador - Identificador único do alerta
 ...
 * @param {string} userId - ID do usuário
 * @param {boolean} eLembreteFinal - Se é o lembrete final (11h)
 * @returns {Promise<string|null>} Mensagem formatada ou null se não há alertas
 */
async function buscarAlertasVencimento(userId, eLembreteFinal = false) {
  try {
    const alertas = await buscarAlertasDoDia(userId);
 ...
 * @param {string} userId - ID do usuário
 * @param {boolean} eLembreteFinal - Se é o lembrete final
 * @returns {Promise<string|null>} Mensagem de alerta ou null
 */
async function buscarAlertaPremium(userId, eLembreteFinal = false) {
  try {
    ...
    const diasRestantes = Math.ceil((new Date(usuario.data_expiracao_premium) - new Date()) / (1000 * 60 * 60 * 24));
 ...
 * @param {string} userId - ID do usuário
 * @param {boolean} eLembreteFinal - Se é o lembrete final (11h)
 * @returns {Promise<string|null>} Mensagem completa ou null
 */
async function buscarTodosAlertas(userId, eLembreteFinal = false) {
  try {
    ...
    logger.error('Erro ao buscar todos os alertas:', error);
    return null;
  }
}
 ...
 * @param {string} userId - ID do usuário
 * @returns {Promise<boolean>} True se há alertas
 */
 ...
 * @param {boolean} eLembreteFinal - Se é o lembrete final (11h)
 * @returns {Promise<{enviados: number, erros: number}>} Estatísticas
 */
 ...
 * @returns {boolean} True se está no horário permitido
 */
 ...
 * @returns {boolean} True se é 8h da manhã
 */
 ...
 * @returns {boolean} True se é 11h da manhã
 */
