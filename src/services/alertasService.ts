 ...
 */
function gerarIdCartao(cartao: Cartao): string {
  const hoje = new Date();
 ...
 * @returns Identificador único
 */
function gerarIdBoleto(boleto: Boleto): string {
  const hoje = new Date();
 ...
 */
async function buscarAlertasVencimento(userId: string, eLembreteFinal: boolean = false): Promise<string | null> {
  try {
    ...
    let alertasFiltrados: AlertasFiltrados = {
      cartoes: [],
      boletos: [],
      temAlertas: false
    };
 ...
 */
async function buscarAlertaPremium(userId: string, eLembreteFinal: boolean = false): Promise<string | null> {
  try {
 ...
 */
async function temAlertas(userId: string): Promise<boolean> {
  try {
    const alertas = await buscarAlertasDoDia(userId);
    const usuariosExpiracao = await buscarUsuariosPremiumExpiracao(7);
 ...
 */
function estaNoHorarioAlertas(): boolean {
 ...
 */
function ePrimeiraVerificacaoDoDia(): boolean {
  const agora = new Date();
  const hora = agora.getHours();
 ...
 */
function eVerificacaoFinalDoDia(): boolean {
  const agora = new Date();
  const hora = agora.getHours();
  const diaSemana = agora.getDay();
